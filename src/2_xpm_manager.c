...
/*   Updated: 2025/03/29 00:52:55 by afodil-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_textures(t_data *data)
{
	...
	if (!data->player.img)
		ft_printf("Erreur: PLAYER.xpm\n");

	data->wall.img = mlx_xpm_file_to_image(data->mlx, "assets/BRICK1.xpm",
			&data->wall.width, &data->wall.height);
	if (!data->wall.img)
		ft_printf("Erreur: BRICK1.xpm\n");

	data->floor.img = mlx_xpm_file_to_image(data->mlx, "assets/FLOOR.xpm",
			&data->floor.width, &data->floor.height);
	if (!data->floor.img)
		ft_printf("Erreur: FLOOR.xpm\n");

	data->collectible.img = mlx_xpm_file_to_image(data->mlx, "assets/COLLECTIBLES1.xpm",
			&data->collectible.width, &data->collectible.height);
	if (!data->collectible.img)
		ft_printf("Erreur: COLLECTIBLES1.xpm\n");

	data->exit.img = mlx_xpm_file_to_image(data->mlx, "assets/EXIT.xpm",
			&data->exit.width, &data->exit.height);
	if (!data->exit.img)
		ft_printf("Erreur: EXIT.xpm\n");

	// Vérifie toutes les textures
	if (!data->player.img || !data->wall.img || !data->floor.img
		|| !data->collectible.img || !data->exit.img)
	{
		ft_print_error("Error: Failed to load one or more textures");
		exit(ERROR);
	}
}
